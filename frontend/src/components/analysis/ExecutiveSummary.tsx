...
export function ExecutiveSummary({ thesis, businessSignals, summary, metadata, lang = 'en' }: ExecutiveSummaryProps) {
    const isZh = lang === 'zh';
    const generatedAtTime = metadata?.generatedAt
        ? new Date(metadata.generatedAt)
        : null;
    const generatedAtLabel = generatedAtTime && !Number.isNaN(generatedAtTime.getTime())
        ? generatedAtTime.toLocaleString(isZh ? 'zh-CN' : undefined)
        : null;
                    ...
                    <div className="flex h-full flex-col justify-between gap-6">
                        ...
                        {metadata && (
                            <p className="text-xs text-slate-500">
                                {isZh
                                    ? `由 ${metadata.modelName}${generatedAtLabel ? ` 生成于 ${generatedAtLabel}` : ' 生成'} (${metadata.language === 'zh' ? '中文' : metadata.language})`
                                    : `Generated by ${metadata.modelName}${generatedAtLabel ? ` at ${generatedAtLabel}` : ''} (${metadata.language})`}
                            </p>
                        )}
                    </div>
                </div>
