...
当前主链路已经把 evidence-aware generation 放在 Python Research Service 内完成。
三类 MVP task 都输出 typed `taskSections`，并由 LLM final synthesizer 基于 evidence
memory 生成：

- Latest Earnings：SEC companyfacts、RAG filing evidence、metric evidence -> LLM latest earnings report。
- Business Driver：RAG filing evidence、source-bound business signals -> LLM business driver report。
- Cash Flow：SEC companyfacts、RAG filing evidence、metric evidence -> LLM cash-quality and capital allocation report。

LLM 只能引用 Agent 已检索到的 `source_id`。如果 provider 输出未知 source id，系统必须
拒绝该 synthesis 并走 degraded/fallback，而不是把未验证 citation 透传给前端。
...
| Backend | Spring Boot, WebFlux, JPA | 主 API、SSE relay、history、SEC filing fetch、financial data APIs、Python Agent client |
...
| Agent Runtime | Typed bounded workflow | bounded tool loop、policy、coverage stop、tool events |
...
LlamaIndex 是 RAG 主框架。当前生产 Agent Runtime 是 typed bounded workflow；
LangGraph/graph-style workflow 可以作为后续编排实现参考，但不作为前端或 Spring Boot
contract。Spring Boot 是产品主后端。Next.js 是产品前端。Postgres / PGVector 是第一版统一持久化和向量存储。
...
生产分析请求只走 Python Research Service。离线 RAG baseline、PGVector smoke 和 eval artifact 可以并存，但必须通过 eval runner 或 gated smoke 运行，不能让同一个用户分析请求混用多套报告生成路径。

### Agent 可解释性边界

用户需要看到可信度解释，而不是 raw debug trace。前端默认展示 source status、evidence count、citation coverage、source context 和 human-readable degraded reason。`AgentEvent`、`planner_context`、tool budget、fallback counter 等结构化 trace 主要服务 eval、release gate、内部 debug panel 和线上支持。

任何用户可见解释都不得暴露原始 chain-of-thought、provider private prompts、hidden scratchpad、provider key、长 filing 原文或完整 prompt payload。高级/内部 debug panel 可以展开结构化 Agent trace，但只能显示 phase、tool name、status、latency、decision summary、coverage summary 和 degraded reason。
...
- Typed bounded workflow 能执行 task policy、tool calls、coverage stop，并输出结构化 agent events。
- Java 后端可以调用 Python Research Service，并将最终 typed report 映射给前端。
- Agent 失败时必须返回明确 unavailable/degraded 状态。
- 三个 MVP task 都可以在 provider tool E2E gate 中完成 LLM final synthesis。
...
- Java report-generation fallback 已移除。
- Python Research Service 是生产默认分析路径。
