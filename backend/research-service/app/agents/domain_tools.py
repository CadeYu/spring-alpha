...
class YahooCompanyProfileProvider:
    ...
    def fetch_company_profile(self, *, ticker: str) -> dict[str, object]:
        ...
        business_summary = _yahoo_raw_value(
            asset_profile.get("longBusinessSummary") or asset_profile.get("description")
        )
...
class ResearchToolService:
    ...
    def get_company_facts(self, tool_input: CompanyFactsInput, state: AgentState) -> ToolResult:
        ...
        if self._facts_provider is not None:
            ...
            missing_metrics = [str(metric) for metric in missing_metric_values if str(metric)]
            if missing_metrics:
...
class LlamaIndexResearchToolService(ResearchToolService):
    ...
    def search_filing_sections(
        ...
        retrieved_nodes = _dedupe_retrieved_nodes(
            node for result in results for node in _retrieved_nodes(result)
        )
...
def _task_scoped_filing_queries(query: str, state: AgentState) -> list[str]:
    ...
    if state.task_type == ResearchTaskType.CASH_FLOW_CAPITAL_ALLOCATION:
        return [
            (
                "operating cash flow capital expenditures buybacks dividends debt "
                "liquidity working capital capital allocation"
            ),
            (
                "net cash provided by operating activities capex share repurchases "
                "capital return program debt maturities cash equivalents"
            ),
        ]
            ...
            " ".join(
                [
                    ...
                    (
                        ...
                        "capital expenditures capex share repurchases buybacks dividends "
                        "debt liquidity working capital"
                    ),
                ]
            )
        ]
