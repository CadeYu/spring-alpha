...
public class ResearchAgentReportMapper {
    ...
    public AnalysisReport toAnalysisReport(ResearchAgentResult result, String language) {
        ...
        return AnalysisReport.builder()
                ...
                .sourceContext(sourceContext(result, finalReport))
    ...
    private AnalysisReport degradedAnalysisReport(ResearchAgentResult result, String language) {
        ...
        return AnalysisReport.builder()
                ...
                .sourceContext(sourceContext(result, Map.of()))
                .metadata(AnalysisReport.AnalysisMetadata.builder()
                        .modelName("python-research-service")
                        .generatedAt(Instant.now().toString())
                        .language(language)
    ...
    private AnalysisReport.SourceContext sourceContext(ResearchAgentResult result, Map<String, Object> finalReport) {
        ...
        boolean optionalSentimentOnly = completed && isOptionalSentimentSourceOnly(result, finalReport);
        String status = completed
                ? (degradedReasons.isEmpty() || optionalSentimentOnly ? "GROUNDED" : "LIMITED")
                : "DEGRADED";
        String message = sourceContextMessage(degradedReasons, completed, optionalSentimentOnly);
        if (message.isBlank()) {
    ...
    private String sourceContextMessage(
            List<String> degradedReasons,
            boolean completed,
            boolean optionalSentimentOnly) {
        if (completed && degradedReasons.isEmpty()) {
            return "Generated by Python Research Service.";
        }
        String joinedReasons = String.join("; ", degradedReasons);
        if (optionalSentimentOnly && !joinedReasons.isBlank()) {
            return "Reddit source limited: " + joinedReasons;
        }
        return joinedReasons;
    }

    private boolean isOptionalSentimentSourceOnly(
            ResearchAgentResult result,
            Map<String, Object> finalReport) {
        ResearchTaskType taskType = result.taskType();
        if (taskType == null) {
            Map<String, Object> rawSections = mapValue(finalReport.get("task_sections"));
            if (rawSections.isEmpty()) {
                rawSections = mapValue(finalReport.get("taskSections"));
            ...
            if (!rawSections.isEmpty()) {
                taskType = taskType(result, rawSections);
            }
        }
        if (taskType != ResearchTaskType.BUSINESS_DRIVER_DEEP_DIVE || result.events() == null
                || result.events().isEmpty()) {
            return false;
        }
        boolean yahooOk = false;
        boolean stocktwitsOk = false;
        boolean hasNonRedditDegraded = false;
        boolean hasRedditDegraded = false;
        for (ResearchAgentEvent event : result.events()) {
            String toolName = event.toolName();
            String status = event.status();
            if ("fetch_yahoo_news".equals(toolName) && "ok".equalsIgnoreCase(status)) {
                yahooOk = true;
            } else if ("fetch_stocktwits".equals(toolName) && "ok".equalsIgnoreCase(status)) {
                stocktwitsOk = true;
            } else if ("fetch_reddit".equals(toolName) && "degraded".equalsIgnoreCase(status)) {
                hasRedditDegraded = true;
            } else if ("degraded".equalsIgnoreCase(status)) {
                hasNonRedditDegraded = true;
            }
        }
        return yahooOk && stocktwitsOk && hasRedditDegraded && !hasNonRedditDegraded;
    }
