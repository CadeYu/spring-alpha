...
public class MockStrategy implements AiAnalysisStrategy {
    ...
    public Flux<String> analyze(String ticker, String textContent, String lang) {
        log.info("🤖 使用策略: Mock Strategy (本地模拟数据) Lang: {}", lang);
        ...
        if (isChinese) {
            ...
            mockResponse = """
                ### 📊 Analysis Report for %s (Mock Mode)
                
                > **Note**: This report is generated by the local fallback strategy because no external API keys were provided or the API is rate-limited.
                
                #### 1. Key Financials 💰
                *   **Revenue**: $100 Billion (📈 +5%% YoY)
                *   **Net Income**: $25 Billion (Flat)
                *   **EPS**: $6.50
                
                #### 2. Risk Factors ⚠️
                *   **Macroeconomic**: Inflationary pressures affecting consumer spending.
                *   **Supply Chain**: Potential disruptions in global logistics.
                *   **Regulatory**: Increasing scrutiny on antitrust matters.
                
                #### 3. Analyst Verdict 👨‍⚖️
                The company shows **resilient** performance despite headwinds. We recommend a **HOLD** rating until uncertainty clears.
                """.formatted(ticker);
        }
